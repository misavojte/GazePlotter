...
* **Metrics**: Added [eye-movement type metrics](/docs/metrics/eye-movement), measuring what the eyes were doing rather than where they looked: per-type counts, time budgets, segment durations, and time to first eye movement (saccadic latency, read at the Saccade type). Types merged in the [Eye-movement Type Library](/docs/workspace/eye-movement-type-library) are measured as one, and each metric can also be picked at a single type for the scalar plots.
* **Plots**: Added the [Eye-movement Comparison](/docs/visualizations/eye-movement-comparison) plot, one distribution per eye-movement type with the same overlays and controls as [AOI Comparison](/docs/visualizations/aoi-comparison). [Metric Timeline](/docs/visualizations/metric-timeline) now paints only where its metric actually measured.
* **Selections**: The [eye-movement type](/docs/workspace/eye-movement-type-library) and [event](/docs/workspace/event-library) libraries now ship a ready-made selection for the narrowed case, *Just fixations* and *No events*, in place of the built-in picker options. Both are ordinary selections, so you can rename, edit, or delete them, and older workspaces are pointed at them automatically.
* **Linked Hovering**: Hovering one plot marks the same moment and the same participant in the others. Moments stay within one stimulus, participants cross stimuli, and a pair (a similarity cell, a network edge, a recurrence cell) marks both members. See [Linked Hovering Across Plots](/docs/visualizations/#linked-hovering-across-plots).
* **Scanpath Similarity**: The [ScanGraph](/docs/visualizations/scanpath-similarity) network is now scored as in Doležalová and Popelka (2016), so its threshold is the article's similarity parameter *p* and comparable with the standalone tool. It also gains the article's percentage-of-edges reading of that threshold and its clique groups. Levenshtein and Needleman-Wunsch are separate metrics now, each with a collapsed variant, and the [ScanGraph export](/docs/export/scangraph) offers either string form.
* **Internals**: Reorganized the comparison plots around a shared distribution layer, unified the plot highlight marks, and extracted the workspace canvas behaviours into their own modules.
