...
- **[Event Comparison](/docs/visualizations/event-comparison/)**: The same comparison along the event axis: one distribution per event channel for occurrence counts, active time, time shares, durations, and onset latencies. Offered when the dataset carries event data.
