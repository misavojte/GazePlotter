...
The **Sort** and **Bulk actions** buttons above the list work similarly to other libraries, allowing you to sort categories by name or rename items using regular expressions. Select rows to merge or split them, or save them as a named selection in the tray below the list (visibility is retired; a selection replaces it). Each plot picks a selection in its pane's **Eye-movement Types** section to narrow which non-fixation categories it draws — an empty selection acts as a "Fixations only" filter.
