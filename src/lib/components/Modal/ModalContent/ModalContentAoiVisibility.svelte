...
const participantOptions = [{ label: 'To all', value: 'all' }].concat(getParticipants().map(participant => {
  return {
...
const handleSubmit = () => {
  ...
  files[0].text().then(x => {
    ...
    const participantId = selectedParticipantId === 'all' ? null : parseInt(selectedParticipantId)
...
<div class="content">
    <GeneralSelectBase label="For stimulus" options={stimuliOptions} bind:value={selectedStimulusId} />
    <GeneralSelectBase label="For participant" options={participantOptions} bind:value={selectedParticipantId} />
