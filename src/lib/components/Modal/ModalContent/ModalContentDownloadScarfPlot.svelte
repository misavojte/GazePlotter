<script lang="ts">
  ...
  const handleSubmit = () => {
    ...
    if (!scarfPlot) throw new Error(`No scarf plot with id ${scarfId} found in DOM.`)
    const scarfPlotDownloader = new ScarfDownloader(fileName, typeOfExport, width, scarfPlot)
...
<GeneralSelectBase label="Output file type" options={options} bind:value={typeOfExport} />
