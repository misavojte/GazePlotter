<script lang="ts">
    import {
        getScarfPlotState,
        getStimulusId,
        getStimulusLastValue, scarfPlotStates,
        updateStimulusLastValue
    } from '$lib/stores/scarfPlotsStore.js'
  import { getStimuliOrderVector } from '$lib/stores/dataStore.js'
  import MajorButton from '../../General/GeneralButton/GeneralButtonMajor.svelte'
  import GeneralFieldset from '../../General/GeneralFieldset/GeneralFieldset.svelte'
...
<GeneralFieldset legend="Absolute timeline [ms]">
    <GeneralInputNumber legend="Last value (0 = automatic)" bind:value={absoluteVal} />
    <GeneralRadio
        options={[{ value: 'this_stimulus', label: 'This stimulus' }, { value: 'all_stimuli', label: 'All stimuli' }]}
        legend="Apply to"
        bind:userSelected={absoluteTimelineApply}
    />
</GeneralFieldset>
<GeneralFieldset legend="Ordinal timeline [indices]">
    <GeneralInputNumber legend="Last value (0 = automatic)" bind:value={ordinalVal} />
    <GeneralRadio
        options={[{ value: 'this_stimulus', label: 'This stimulus' }, { value: 'all_stimuli', label: 'All stimuli' }]}
        legend="Apply to"
