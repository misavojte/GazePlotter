<script lang="ts">
    ...
    import {
      duplicateScarfPlotState, getScarfPlotState,
    ...
    const openClipModal = () => {
      modalStore.open(ModalContentScarfPlotClip, 'Clip scarf timeline', { scarfId })
    }

    const openAoiModificationModal = () => {
      modalStore.open(ModalContentAoiModification, 'AOI customization', { selectedStimulus: currentStimulusId.toString() })
    }
    ...
    const downloadPlot = () => {
      modalStore.open(ModalContentDownloadScarfPlot, 'Download scarf plot', { scarfId })
    }
