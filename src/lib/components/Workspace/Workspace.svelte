<script lang="ts">
  ...
  const getUtilityFilling = (
    type: 'load' | 'empty'
  ): WorkspaceFillingType[] => {
    return [
      {
        id: getUniqueId(),
        content: type,
  ...
  const getFillingForSingleScarfPlot = (
    scarfPlotSettings: ScarfSettingsType
  ): WorkspaceFillingType => {
    return {
...
<style>
  ...
  .isSmall.center {
    ...
    grid-column-end: calc(
      (var(--span-large) - var(--span-small)) / 2 + var(--span-small) + 1
    );
  }
  ...
  .isMedium.center {
    ...
    grid-column-end: calc(
      (var(--span-large) - var(--span-medium)) / 2 + var(--span-medium) + 1
    );
  }
