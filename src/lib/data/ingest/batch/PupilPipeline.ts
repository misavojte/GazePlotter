...
export function stripTimestampFromRecordingName(recordingName: string): string {
  ...
  const timestampPattern =
    /_$libd{4}-$libd{2}-$libd{2}_$libd{2}:$libd{2}:$libd{2}$/
...
export function deriveStimulusFromZipName(zipName: string): string {
  return zipName.replace(/$lib.zip$/i, '')
}
...
export function forEachCsvRow(
  ...
  for (let i = 0; i < csv.length; i++) {
    if (csv.charCodeAt(i) === 10 /* $libn */) {
      const line = csv.slice(start, i).replace(/$libr$/, '')
      start = i + 1
  ...
  if (start < csv.length) {
    const line = csv.slice(start).replace(/$libr$/, '')
