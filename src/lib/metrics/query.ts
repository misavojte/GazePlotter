...
import { runRecipe, runIndividuals, type Scope } from './core/runtime'
...
export type MetricResult =
  | { shape: 'scalar';          metricId: string; unit: string; value: number; isFinite: boolean }
  | { shape: 'aoi-vector';      metricId: string; unit: string; values: number[]; slots: AoiSlotInfo }
  | { shape: 'aoi-pair-matrix'; metricId: string; unit: string; matrix: number[]; size: number }
...
export function query(instance: MetricInstance, scope: Scope): MetricResult {
  ...
  if (!recipe) return emptyResult(instance.baseId, 'scalar', '')
  const raw = runRecipe(recipe, instance, scope)
  return wrapResult(recipe.id, recipe.unit, recipe.outputShape, raw, scope)
}
...
export function queryBatch(instances: readonly MetricInstance[], scope: Scope): Map<number, MetricResult> {
  ...
  if (plain.length > 0) {
    ...
    for (const inst of plain) {
      ...
      if (!raw) continue
      out.set(inst.id, wrapResult(recipe.id, recipe.unit, recipe.outputShape, raw, scope))
    }
  }
...
export function queryGroup(instance: MetricInstance, group: GroupScope): MetricResult {
  ...
  return wrapResult(recipe.id, recipe.unit, recipe.outputShape, reduced, scope)
}
...
function wrapResult(metricId: string, unit: string, shape: OutputShape, raw: number[], scope: Scope): MetricResult {
  if (shape === 'scalar') {
    const v = raw[0] ?? Number.NaN
    return { shape: 'scalar', metricId, unit, value: v, isFinite: Number.isFinite(v) }
  }
  if (shape === 'aoi-vector') {
    const slots = deriveSlotInfo(scope, raw.length)
    return { shape: 'aoi-vector', metricId, unit, values: raw, slots }
  }
  const size = Math.round(Math.sqrt(raw.length))
  return { shape: 'aoi-pair-matrix', metricId, unit, matrix: raw, size }
}

function deriveSlotInfo(scope: Scope, totalSlots: number): AoiSlotInfo {
  const noAoiSlot = totalSlots - 2
  const anyFixationSlot = totalSlots - 1
  return { totalSlots, noAoiSlot, anyFixationSlot }
}

function emptyResult(metricId: string, shape: OutputShape, unit: string): MetricResult {
  if (shape === 'scalar') return { shape: 'scalar', metricId, unit, value: Number.NaN, isFinite: false }
  if (shape === 'aoi-vector') return { shape: 'aoi-vector', metricId, unit, values: [], slots: { totalSlots: 0, noAoiSlot: 0, anyFixationSlot: 0 } }
  return { shape: 'aoi-pair-matrix', metricId, unit, matrix: [], size: 0 }
}
