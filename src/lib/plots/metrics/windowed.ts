...
import { computeParticipantScalar } from './compute'
...
function epochRqa(
  ...
  for (let start = 0; start + windowSize <= seq.length; start += windowSize) {
    ...
    if (sub.length >= MIN_FIXATIONS_RQA)
      scalars.push(computeRqaAoiScalar(instance, { fixationAoiSequence: sub } as ParticipantBarMetrics))
  }
...
function epochNonRqa(
  ...
  for (let wStart = totalStart; wStart + windowSizeMs <= totalEnd; wStart += windowSizeMs) {
    ...
    if (wMetrics[0])
      scalars.push(computeParticipantScalar(instance, { participantMetrics: wMetrics[0], aoiIndex }))
  }
  return applyReduction(scalars, reduction)
}
