...
export const getScarfPlotState = (scarfStates: ScarfSettingsType[], scarfPlotId: number): ScarfSettingsType | undefined => {
  return scarfStates.find((state) => state.scarfPlotId === scarfPlotId)
}

const getDefinedScarfPlotState = (scarfStates: ScarfSettingsType[], scarfPlotId: number): ScarfSettingsType => {
  const scarfPlotState = getScarfPlotState(scarfStates, scarfPlotId)
  if (scarfPlotState === undefined) throw new Error(`Scarf plot state with id ${scarfPlotId} not found`)
...
export const getStimulusLastValue = (scarfPlotId: number, stimulusId: number, type: 'absolute' | 'ordinal'): number => {
  const scarfStates = get(scarfPlotStates)
...
export const updateTimeline = (scarfPlotId: number, timeline: 'absolute' | 'relative' | 'ordinal'): void => {
  scarfPlotStates.update(
    (prev) => {
...
export const updateStimulusId = (scarfPlotId: number, stimulusId: number): void => {
  scarfPlotStates.update(
    (prev) => {
      const newState = getDefinedScarfPlotState(prev, scarfPlotId)
...
export const updateStimulusLastValue = (scarfPlotId: number, stimulusId: number, lastValue: number, type: 'absolute' | 'ordinal'): void => {
  scarfPlotStates.update(
    (prev) => {
      const newState = getDefinedScarfPlotState(prev, scarfPlotId)
      if (type === 'absolute') {
