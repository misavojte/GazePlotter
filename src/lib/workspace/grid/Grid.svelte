<script lang="ts">
  ...
  const handleItemPreviewMove = (event: { id: number; previewY: number; h: number }) => {
    // Calculate workspace height for preview position
    const requiredHeight = calculateWorkspaceHeight(event.id, event.previewY, event.h)
  ...
  const handleItemMove = (event: { id: number; x: number; y: number }) => {
    const currentItem = gridItems.find((item: AllGridTypes) => item.id === event.id)
    if (currentItem) {
  ...
  const handleItemPreviewResize = (event: { id: number; y: number; h: number }) => {
  ...
  const handleDragEnd = (event: { id: number; x: number; y: number; w: number; h: number; dragComplete: boolean }) => {
    isDragging = false
    draggedItemId = null
  ...
  const handleResizeEnd = (event: { id: number; x: number; y: number; w: number; h: number; resizeComplete: boolean }) => {
    isResizing = false
    resizedItemId = null
  ...
  const handleDragHeightUpdate = (event: { id: number; y: number; h: number }) => {
  ...
  const handleItemRemove = (event: { id: number }) => {
    const itemToRemove = gridItems.find((item: AllGridTypes) => item.id === event.id)
    if (itemToRemove) {
      onWorkspaceCommand({
        type: 'removeGridItem',
  ...
  const handleItemDuplicate = (event: { id: number }) => {
    const itemToDuplicate = gridItems.find((item: AllGridTypes) => item.id === event.id)
    if (itemToDuplicate) {
      onWorkspaceCommand({
  ...
  const handleItemEdgeScroll = (event: {
    ...
    if (itemBounds.right >= viewportBounds.right - edgeThreshold) {
      ...
      grid.temporaryDragWidth = (grid.width / gridCellWidth + AUTO_SCROLL_AMOUNT) * gridCellWidth
    ...
    if (itemBounds.bottom >= viewportBounds.bottom - edgeThreshold) {
      ...
      grid.temporaryDragHeight = (grid.height / gridCellHeight + AUTO_SCROLL_AMOUNT) * gridCellHeight
      ...
      if (!wasAlreadyScrolling) {
        ...
        autoScrollInterval = setInterval(() => {
          ...
          if (scrollDirection.x !== 0) {
            ...
            // Fix: Use linear scaling instead of quadratic for smoother control
            if (scrollDirection.x < 0 && currentX < 150) {
              effectiveMaxSpeed = maxSpeed * (currentX / 150) // Linear scaling
            }
            ...
            currentSpeedX =
              Math.abs(currentSpeedX) > 0.05 // More sensitive stopping threshold
          ...
          if (scrollDirection.y !== 0) {
            ...
            if (scrollDirection.y < 0 && currentY < 150) {
              effectiveMaxSpeed = maxSpeed * (currentY / 150) // Linear scaling
            }
            ...
            currentSpeedY =
              Math.abs(currentSpeedY) > 0.05 // More sensitive stopping threshold
          ...
          if (Math.abs(currentSpeedX) < 0.05 && Math.abs(currentSpeedY) < 0.05) {
            if (autoScrollInterval !== null) {
              clearInterval(autoScrollInterval)
              autoScrollInterval = null
              isAutoScrolling = false
  ...
  // Handle edge detection from WorkspaceItem
