...
/**
 * Creates an undo/redo service for workspace commands.
 * 
 * This service maintains two stacks of command chains:
 ...
 * 
 * @returns Service object with undo/redo operations and state queries
 */
export function createUndoRedoService() {
  // Use writable store for reactive state
  const stateStore = writable<UndoRedoState>({
    undoStack: [],
    redoStack: [],
    pendingChain: null,
  ...
  // Derive reactive boolean values from the state store
  const canUndo = derived(stateStore, $state => $state.undoStack.length > 0)
  const canRedo = derived(stateStore, $state => $state.redoStack.length > 0)
   ...
   */
  function recordCommand(original: WorkspaceCommandChain, reverse: WorkspaceCommandChain | null): void {
   ...
   */
  function undo(): WorkspaceCommandChain[] | null {
   ...
   */
  function redo(): WorkspaceCommandChain[] | null {
    let result: WorkspaceCommandChain[] | null = null
   ...
   */
  function isProcessingUndoRedo(): boolean {
    let result = false
    stateStore.update($state => {
      result = $state.isProcessingUndoRedo
      return $state
    })
    return result
  }
...
/**
 * Type representing the undo/redo service
 */
export type UndoRedoService = ReturnType<typeof createUndoRedoService>
