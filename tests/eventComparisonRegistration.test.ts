/**
 * Registry smoke for the event comparison plot. Lives in its own file so the
 * plots barrel (and with it registry.ts) initializes FIRST, the same order
 * production uses — a test that imports a plot subpath before the barrel
 * would freeze a partially-initialized registry entry instead.
 */
